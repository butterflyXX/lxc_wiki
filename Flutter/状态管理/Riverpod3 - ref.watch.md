...
StateT watch<StateT>(ProviderListenable<StateT> target) {
  ...
  return _dependencies
      ...
      .readSafe().valueOrProviderException as StateT;
}
```
...
在`watch`的最后,返回了`readSafe.valueOrProviderException`
```dart
$Result<StateT> readSafe() {
    final that = impl;

    // 判断providerElement是否需要被销毁, 后边单独会介绍自动销毁的provider再具体分析
    that._listenedElement.mayNeedDispose();

    // 这里会执行flush,如果在_addListener中没有执行,这里再执行,如果是weak的_addListener,并不会执行flush
...
```
这里的`impl`本质就是sub本身,`that._callRead`是返回当前状态,关于`state`这块的内容下一小节展开解读
